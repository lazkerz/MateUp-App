...
class DashboardFragment : Fragment(), ProductView {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val foundationAdapter = FoundationAdapter()
        val eyeshadowAdapter = EyeshadowAdapter()
        val lipstickAdapter = LipstickAdapter()
        val blushAdapter = BlushAdapter()

        rv3.adapter = foundationAdapter
        rv.adapter = eyeshadowAdapter
        rv4.adapter = lipstickAdapter
        rv2.adapter = blushAdapter
        ...
        if (apiServicePersonalized != null) {
            ...
            when(userModel.skinType) {
                ...
                "Dry" -> {
                    ...
                    presenter.getProductPersonalized("cretclean", "Canadian", "powder", "blush")
                }
                else -> Log.e("Presenter", "Can't get presenter")
            }
        } else {
            Log.e("Dashboard", "Failed to initialize ApiServicePersonalized")
            Toast.makeText(requireContext(), "Failed to initialize ApiServicePersonalized", Toast.LENGTH_SHORT)
                .show()
        }
    }
